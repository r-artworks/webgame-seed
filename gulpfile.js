...
var LIBRARIES = [
	{
		...
		files: [
			{ name: 'phaser.min.js' },
			{ name: 'phaser.map', onlyDev: true },
			{ name: 'phaser.js', onlyDev: true }
		]
	}
];
...
/**
 * Copies required libraries into the './build/scripts' folder.
 * This way you can call 'npm update', get the lastest versions and use them on your project with ease.
 */
function copyLibraries() {
	var srcList = [];
	LIBRARIES.forEach(function(library) {
		library.files.forEach(function(file) {
			if (!isProduction() || (isProduction() && !file.onlyDev))
				srcList.push(library.path + file.name);
		});
	});
 ...
 *
 * In order to avoid copying Libraries and Static files on each build,
...
gulp.task('copyLibraries', ['copyStatic'], copyLibraries);
gulp.task('build', ['copyLibraries'], build);
gulp.task('fastBuild', build);
gulp.task('serve', ['build'], serve);
gulp.task('watch-js', ['fastBuild'], browserSync.reload); // Rebuilds and reloads the project when executed.
gulp.task('watch-static', ['copyLibraries'], browserSync.reload);

/**
 * The tasks are executed in the following order:
 * 'cleanBuild' -> 'copyStatic' -> 'copyLibraries' -> 'build' -> 'serve'
 *
